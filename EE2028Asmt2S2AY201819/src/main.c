...
static unsigned char TILT_THRESHOLD_TXT[] = "Poor Landing Altitude \r\n";
...
void setTempMessage() {
	...
	printf("temp: %d %f\n", temp, temp / 10.0);
	sprintf(EXPLORING_MODE_MESSAGE[2], "Temp: %2.1f deg   ", temp / 10.0);
//	msg = "Temp %2.1f deg \r\n", temp/10.0;
...
void sendUARTMessage(int typeOfMsg) {
	...
	case 7:
		...
		break;
	case 8:
		UART_Send(LPC_UART3, (uint8_t *) LIGHT_THRESHOLD_TXT, strlen(LIGHT_THRESHOLD_TXT),
				BLOCKING);
		break;
	}
}
